...
def main():
    for k, j in enumerate(categories):
        ...
        # NOTE : There's chance that some elements are non available each time, due to unsure reason (visibility setting /
        # waiting certain element to load page). Besides, some category will have less data
        ...
        with open(Path(main_folder, 'links_cat_{}.json'.format(k)), 'w') as outfile:
    ...
    for i in range(0, len(categories)):
        with open(Path(main_folder, 'links_cat_{}.json'.format(k))) as outfile:
            f = json.load(outfile)
            product_links.append(f)
    with open(Path(main_folder, 'product_links.json'), 'w') as outfile:
